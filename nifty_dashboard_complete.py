...
CONFIG = {
    ...
    # PATCH-4c: changed from 1 to 3 (every 15 min).
    # At N=1, 75 cycles/session exhausted the 80-call daily cap before close.
    # N=3 uses ~25 calls/session and matches the '15-min analysis' intent.
    'CLAUDE_EVERY_N_CYCLES': 3,
    'PROMPT_VERSION':        'v1.2',
    ...
    # ── VIX thresholds ─────────────────────────────────────────────────────────
    # VIX > 20: elevated vol — option premiums expensive, expansion environment.
    'VIX_HIGH_THRESHOLD':  20.0,
    # Standard NIFTY low-vol zone. VIX < 15 = low IV = premiums compressed = decay-friendly.
    # Changed from 13.0 — VIX < 13 is rare on NIFTY; 15 is the practical threshold.
    'VIX_LOW_THRESHOLD':   15.0,
    'VIX_RISING_LOOKBACK':  5,     # candles used for VIX trend detection

    # ── Decay / expansion environment thresholds ──────────────────────────────────
    # Score range: -9 (full expansion) to +11 (full decay).
    # Threshold of 5 = majority of factors must align. Prevents false triggers.
    # Decay Favorable    : score >= DECAY_SCORE_MIN      → HEDGED_SELL suggested
    # Expansion Favorable: score <= -EXPANSION_SCORE_MIN → BUY suggested
    # Neutral            : between thresholds            → no setup generated
    'DECAY_SCORE_MIN':     5,
    'EXPANSION_SCORE_MIN': 5,
    # Max VIX at which buying is allowed. Above this, IV so elevated that
    # mean-reversion risk outweighs any directional premium gain for a buyer.
    'VIX_BUY_BLOCK':      22.0,
    ...
    # PATCH-2a: was 1, now 0. Resistance shifting UP is price-following and
    # was double-counting directional moves. See PATCH-2b and PATCH-2c.
    'SCORE_RESISTANCE_SHIFT_UP':      0,
...
class ConfigReader:
    ...
    def load(self) -> Dict[str, Any]:
        ...
        self.settings = {
            ...
            # PATCH-3a: renamed 'anthropic_key' → 'openai_key'. The SETTINGS
            # tab cell (B21) is unchanged; only the Python key name is updated.
            'openai_key':       raw.get('anthropic_key', ''),
            'telegram_token':   raw.get('telegram_token', ''),
            'telegram_chat_id': raw.get('telegram_chat_id', ''),
            'lot_size':         max(1, safe_int(raw.get('lot_size', '65'), 65)),
            'contract_lot_size': max(1, safe_int(raw.get('lot_size', '65'), 65)),
        }
        # Push B21-B23 from SETTINGS tab into CONFIG
        # PATCH-3b: key renamed from 'anthropic_key' to 'openai_key'.
        if self.settings['openai_key']:
            CONFIG['OPENAI_API_KEY']    = self.settings['openai_key']
            CONFIG['ANTHROPIC_API_KEY'] = self.settings['openai_key']  # backward-compat alias
        ...
        for rec in raw_data:
            ...
            if inst_type == 'OPTIDX':
                ...
                options_rows.append({
                    ...
                    'lotsize':    int(rec.get('lotsize', 65) or 65),
                })
...
class CandleFetcher:
    ...
    def _fetch_candles(self, exchange: str, token: str, interval: str,
        ...
        if not candle_data or not isinstance(candle_data, list) or len(candle_data) == 0:
            # Angel One transiently returns null data for bars that closed within
            # the last 2-5 minutes (exchange publishing delay). Retry once after
            # a short delay before giving up and triggering FEED-01 FAIL.
            log.warning(
                f"[CandleFetcher.{label}] No candle data returned — retrying in 5s. "
                f"fromdate={params['fromdate']} todate={params['todate']}"
            )
            time.sleep(5)
            try:
                response2 = retry_call(_call, attempts=2, delay=3.0,
                                       fallback=None, label=label + '_retry')
            except Exception:
                response2 = None
            candle_data = (response2 or {}).get('data') if response2 else None
            if not candle_data or not isinstance(candle_data, list) or len(candle_data) == 0:
                log.warning(
                    f"[CandleFetcher.{label}] No candle data after retry (data is null or empty). "
                    f"fromdate={params['fromdate']} todate={params['todate']}"
                )
                return None
    ...
    def fetch_prev_day(self) -> dict:
        ...
        Fix: do NOT use iloc[-2] blindly.
        Before 09:15 IST today has no daily candle → iloc[-1] = yesterday, iloc[-2] = day before.
        After  09:15 IST a partial candle appears  → iloc[-1] = today,     iloc[-2] = yesterday.
        Instead: walk backwards and return the last candle whose date < today.
        """
        try:
            now = now_ist().replace(tzinfo=None)
            ...
            if df is None or df.empty:
                log.warning("[CandleFetcher.fetch_prev_day] No daily candles returned.")
                return {}

            today_date = now_ist().date()

            # Walk backwards to find the last COMPLETED trading day (date < today)
            prev = None
            ...
            if prev is None:
                log.warning("[CandleFetcher.fetch_prev_day] Could not find a completed prior-day candle.")
                return {}

            prev_date  = str(prev['datetime'])[:10]
...
class MetricsCalculator:
    ...
    def _compute_max_score() -> int:
        ...
        return sum(v for v in [
            ...
            # PATCH-2b: SCORE_RESISTANCE_SHIFT_UP = 0, removed from positive sum.
            # The weight is kept in CONFIG for documentation; excluded here so
            # max_score drops by 1 (from 12 to 11) matching the corrected logic.
    ...
    def compute(self, chain: pd.DataFrame, prev_metrics: Optional[dict] = None) -> dict:
        ...
        if fresh_ce_chg == 0 and fresh_pe_chg == 0:
            ...
            # PE writing with CE unwinding = maximum bullish confirmation.
            # Ratio is technically infinite — cap at 5.0 so downstream
            # comparisons against PCR_BULLISH_THRESHOLD (1.2) always fire.
            pcr_chg_oi = 5.0
        elif fresh_pe_chg == 0:
            # CE writing with PE unwinding = maximum bearish confirmation.
            # Zero triggers pcr_oi <= PCR_BEARISH_THRESHOLD (0.8) correctly.
            pcr_chg_oi = 0.0
        else:
...
class SignalEngine:
    ...
    def run(
        ...
        # PATCH-2c: resistance shift UP is not scored. When the CE wall moves
        # higher it is price-following, not an independent bullish signal.
        # Scoring +1 here double-counted the same move already captured by
        # price structure (+2) and support shift (+1). Only DOWN is scored.
        if resistance_shift < 0:
            score += CONFIG['SCORE_RESISTANCE_SHIFT_DOWN']
        # resistance_shift > 0: not scored (SCORE_RESISTANCE_SHIFT_UP set to 0)
    ...
    def _compute_price_structure(self, spot_df: Optional[pd.DataFrame]) -> str:
        ...
        try:
            ...
            # Use at least 60% agreement, minimum 2 comparisons required.
            # math.ceil keeps proportion consistent regardless of candle count:
            # 4 candles → need 2/3 (67%), 5 candles → need 3/4 (75%),
            # 6 candles → need 3/5 (60%). Avoids the 100% strictness at 4 bars.
            threshold = max(2, math.ceil(n * 0.6))
    ...
    def _compute_vwap(
        ...
        if futures_df is None or futures_df.empty or len(futures_df) < 3:
            # Fewer than 3 candles: VWAP ≈ single typical price ≈ spot — no signal value.
            return None, 'NEUTRAL'
        try:
            df = futures_df.copy()
...
class ThetaEnvironmentAnalyzer:
    ...
    def _compute(self, vix_data, days_to_expiry, spot_df, metrics, signals, sr_event):
        ...
        # Factor 3 — Time to expiry (4 standard NIFTY theta decay bands)
        # DTE  0-2 : peak theta    (+3) — steepest curve, final days of contract
        # DTE  3-7 : strong theta  (+2) — primary selling window for weekly options
        # DTE 8-14 : moderate      (+1) — theta building; selling viable with range
        # DTE  >=15: slow decay    (-1) — not a premium-selling environment
        if days_to_expiry <= 2:
            decay_score += 3   # peak theta
        elif days_to_expiry <= 7:
            decay_score += 2   # strong theta — weekly selling window
        elif days_to_expiry <= 14:
            decay_score += 1   # moderate theta
        elif days_to_expiry >= 15:
            decay_score -= 1   # slow decay

        # Factor 4 — Price behavior: RANGEBOUND +2, REVERSAL +1, TRENDING -1, BREAKOUT/BREAKDOWN -2
        ...
        if price_structure == 'RANGEBOUND':
            ...
            # Reversal at S/R confirms range is holding — decay-friendly (+1 standard rule)
            decay_score += 1
        ...
        # Market condition — standard thresholds for NIFTY options environment.
        # Score range approx -9 (full expansion) to +11 (full decay).
        # Threshold of 5 requires majority of factors to align before triggering.
        if decay_score >= CONFIG.get('DECAY_SCORE_MIN', 5):
            market_condition = 'Decay Favorable'
        elif decay_score <= -CONFIG.get('EXPANSION_SCORE_MIN', 5):
        ...
        reasoning = [
            ...
            (f"{days_to_expiry} days to expiry — "
             + ('peak theta: final 2 days — steepest time-decay curve for NIFTY options'
                if days_to_expiry <= 2
                else 'strong theta zone (3-7 days) — primary selling window for weekly expiry'
                if days_to_expiry <= 7
                else 'moderate theta building (8-14 days) — selling viable with tight S/R'
                if days_to_expiry <= 14
                else 'slow decay (15+ days) — not a premium-selling environment')),
...
class SetupEvaluator:
        ...
        try:
            ...
            if market_condition == 'Decay Favorable':
                ...
                if (expiry_day and
                        ...
                        _vix <= CONFIG.get('VIX_BUY_BLOCK', 22.0)):
                    setup_type = self.SETUP_BUY_BREAK

            elif market_condition == 'Expansion Favorable':
                if _vix > CONFIG.get('VIX_BUY_BLOCK', 22.0):
                    return self._blocked(signal_id,
                        f'VIX {_vix:.1f} > {CONFIG.get("VIX_BUY_BLOCK",22.0):.0f} — premiums too expensive to buy.',
                        expiry_day, pre_expiry_warn, oi_regime,
                        liquidity_ok, late_entry, time_block,
                        gate_results, lot_size)
            ...
            # PATCH-5a: Compute a VIX-adjusted preview hedge gap that mirrors
            # OrderExecutor._compute_hedge_gap() thresholds exactly so the SIGNAL
            # tab max_risk matches the gap used at execution time.
            # VIX > 22 → 400pts | VIX > 18 → 300pts | else → 200pts
            _preview_vix = float(vix_data.get('vix_current', 14.0) or 14.0)
            if math.isnan(_preview_vix):
                _preview_vix = 14.0
            if _preview_vix > 22.0:
                _preview_hedge_gap = 400
            elif _preview_vix > 18.0:
                _preview_hedge_gap = 300
            else:
                _preview_hedge_gap = 200
            _preview_hedge_gap = max(150, (_preview_hedge_gap // 50) * 50)

            if setup_type == self.SETUP_DECAY:
                if 'BULLISH' in bias_upper:
                    short_option_type = 'PE'
                    short_strike      = support_s
                    hedge_strike      = short_strike - _preview_hedge_gap
                else:
                ...
                if (sr_ev in ('BREAKOUT_ABOVE_RESISTANCE',
                    ...
                    short_strike      = support_s
                hedge_strike       = 0.0
                _preview_hedge_gap = 0  # no hedge leg for buy setups
            else:
                short_option_type  = ''
                short_strike       = 0.0
                hedge_strike       = 0.0
                _preview_hedge_gap = 0
            ...
            max_risk = round(
                # PATCH-5b: use _preview_hedge_gap (VIX-adjusted, set in PATCH-5a)
                # instead of the hardcoded 200. This keeps the SIGNAL tab risk
                # figure consistent with what OrderExecutor will actually use.
                _preview_hedge_gap - net_premium if setup_type == self.SETUP_DECAY
                else short_ltp, 2)
            ...
            return {
                ...
                'preview_hedge_gap': _preview_hedge_gap,  # VIX-adjusted gap; 0 for buy setups
            }
        except Exception as e:
    ...
    def _safe_none(self, lot_size=65) -> dict:
        return {
...
class SheetsWriter:
    ...
    CLAUDE_HEADER = [
        ...
        'Call Type',               # ANALYSIS | MORNING | EOD | FALLBACK
        'Minutes Since Last Call', # 0 on first call; increases each cycle GPT skipped
    ]
    ...
    def append_claude_analysis(self, analysis: dict, snapshot: dict, ts: str,
        ...
        try:
            ...
            row = [
                ...
                call_type,                        # ANALYSIS | MORNING | EOD | FALLBACK
                safe_val(minutes_since_last_call), # minutes since last GPT call (0 = first)
            ]
    ...
    def write_settings_help(self):
        try:
            ...
            content = [
                ...
                ['API Key',  'Set in SETTINGS tab B21. Get key at platform.openai.com/api-keys.'],
                ['Call Types',
                 '1) Morning Brief (first cycle, once/session)  '
                 '2) 15-min Analysis (every 3 cycles = every 15 min)  '
                 '3) EOD Review (>=15:30 IST)'],
                ['Output Schema', 'market_bias, strength, signal, reasoning, score_agreement, premium_environment'],
                ['Score Agreement','True if GPT market_bias directionally matches rule engine bias.'],
                ['Unusual Obs.',  'Notable pattern not captured by the rule engine (or null).'],
                ['Premium Env.',  'GPT narrative on market_condition, volatility_context, key_insight.'],
                ['Disable',       'Leave B21 (OpenAI API Key) empty — program runs without GPT analysis.'],
                ['── TELEGRAM (OPTIONAL) ──'],
                ...
                ['Decay Score',
                 'Range approx -9 to +11. '
                 f'>=+{CONFIG.get("DECAY_SCORE_MIN",5)} = Decay Favorable. '
                 f'<=-{CONFIG.get("EXPANSION_SCORE_MIN",5)} = Expansion Favorable.'],
                ['Days to Expiry',
                 '<=2 days = peak theta (+3). '
                 '3-7 days = strong theta (+2). '
                 '8-14 days = moderate (+1). '
                 '>=15 days = slow decay (-1).'],
    ...
    def write_settings_template(self):
        ...
        try:
            ...
            rows = [
                ...
                # Row 21: OpenAI API Key (optional)
                ['OpenAI API Key (B21)', '', 'Optional. From platform.openai.com/api-keys. Leave empty to disable AI analysis. Model: gpt-4.1-mini.'],
                ...
                ['── SIGNAL & EXECUTION ──', '', ''],
                # Row 26: Lot Size
                ['Lot Size', '65', 'NIFTY lot size (currently 65, effective Jan 2026 expiry). Update if SEBI changes it.'],
                # Row 27: warning
    ...
    def append_position_log(self, position: dict, event: str, snapshot: dict):
        try:
            ...
            row = [ist_str(),event,pos.get('action',''),pos.get('short_symbol',''),sv(pos.get('short_strike','')),
                  ...
                  pos.get('lot_size',65),sv(pos.get('short_fill','')),sv(pos.get('hedge_fill','')),
    ...
    def append_signal_row(self, setup, snapshot, signals, metrics, theta_env, vix_data, sr_event,
        ...
        try:
            ...
            row = [setup.get('signal_id',''),ist_str(),cycle_num,setup.get('time_block',''),
                  ...
                  sv(setup.get('net_premium',0)),sv(setup.get('max_risk',0)),setup.get('lot_size',65),
        ...
        try:
            ...
            for sig in pending_signals:
                ...
                def _label(spot_now):
                    ...
                    # FIX-5: Raised threshold from 10 to 25 points.
                    # NIFTY average 5-minute candle body is 20-40 pts in normal
                    # sessions. A 10-pt threshold is below the noise floor —
                    # CORRECT/INCORRECT labels at that level are near-random, and
                    # the quality score adjustments (+15/+40) then corrupt the
                    # setup quality metric shown in the SIGNAL tab.
                    ok = (is_bull and pts > 25) or (is_bear and pts < -25)
                    if abs(pts) <= 25:
                        lbl = 'NO_EDGE'
                    elif ok:
                        lbl = 'CORRECT'
...
class OrderExecutor:
    ...
    def _compute_pnl(self, pos: dict, short_current: float, hedge_current: float) -> float:
        qty = int(pos.get('quantity', 65) or 65)
    ...
    def _evaluate_lifecycle(self, pos: dict, snapshot: dict) -> str:
        if not pos: return ''
        try:
            ist_now = now_ist()
            ...
            # FIX-2: entry_spot must be read from pos before it is used.
            # It is not a parameter or class attribute of _evaluate_lifecycle.
            # Without this, every lifecycle call where snapshot["spot"] is
            # falsy raises NameError caught silently, returning STATE_OPEN
            # and making stoploss, target, and all P&L exits permanently dead.
            ...
            # FIX-1: Use spot_now for moneyness — not entry_spot.
            # entry_spot freezes delta at the moment of entry. As spot moves ITM,
            # actual option delta grows (e.g., 0.25 → 0.85 for a CE gone deep ITM)
            # but the proxy kept using the entry-time estimate, understating loss
            # by up to 80% and causing auto-exits to fire far too late.
            # Fallback to entry_spot if spot_now is unavailable this cycle.
            _ref_spot = spot_now if spot_now > 0 else entry_spot
            if _ref_spot > 0:
                short_strike = float(self.active_position.get('short_strike', _ref_spot) or _ref_spot)
                moneyness = abs(short_strike - _ref_spot) / _ref_spot
                est_delta = max(0.10, 0.50 * math.exp(-moneyness / 0.015))
            else:
                est_delta = 0.50
            # delta_direction: +1 for CE (price rises with spot), -1 for PE (price falls with spot).
            # Without this, pnl_proxy is inverted for SELL_PE — spot falling appears as profit
            # when it is actually a loss, triggering false TARGET and PARTIAL_PROFIT exits.
            if 'PE' in action:
                delta_direction = -1   # PE gains value when spot falls → loss for seller
            else:
                delta_direction = +1   # CE gains value when spot rises → loss for seller
            if txn == 'SELL':
                # Seller loses when option gains value; gains when it decays.
                pnl_proxy = -(spot_move * delta_direction * qty * est_delta)
            else:
                # Buyer gains when option gains value.
                pnl_proxy = (spot_move * delta_direction * qty * est_delta)
            ...
            if entry_structure and pr_struct not in ('', 'INSUFFICIENT_DATA'):
                # For premium sellers the adverse structure flip is when market moves
                # TOWARD their short strike, not away from it.
                # SELL_CE: adverse when structure turns BULLISH (spot rising toward CE strike)
                # SELL_PE: adverse when structure turns BEARISH (spot falling toward PE strike)
                # BUY_CE / BUY_PE: any reversal of entry structure is adverse.
                if txn == 'SELL':
                    if (('CE' in action and 'BULLISH' in pr_struct and 'BEARISH' in entry_structure) or
                        ('CE' in action and 'BULLISH' in pr_struct and 'RANGEBOUND' in entry_structure) or
                        ('PE' in action and 'BEARISH' in pr_struct and 'BULLISH' in entry_structure) or
                        ('PE' in action and 'BEARISH' in pr_struct and 'RANGEBOUND' in entry_structure)):
                        return self.STATE_EXIT_STRUCTURE
                else:
                    # BUY: exit on any structure reversal
            ...
            # FIX-3: OI exit threshold from CONFIG, not a hardcoded constant.
            # COMPARISON_OI_THRESHOLD is used consistently elsewhere; hardcoding
            # here meant CONFIG changes had no effect on lifecycle OI exit.
            OI_THRESHOLD = -CONFIG.get('COMPARISON_OI_THRESHOLD', 50000)
        ...
        try:
            if trigger.get('execute_trade', 'NO').upper() != 'YES':
                if self.active_position:
                    self._run_lifecycle_check(snapshot)
                return False
            ...
            # FIX-5: Separate minimum premium floors for buy vs sell.
            # Previously, `or 'BUY' in action` bypassed the floor entirely for buys.
            _min_sell_prem = 10.0
            _min_buy_prem  = 20.0
            if 'BUY' in action:
                min_prem_ok = (_short_ltp_p >= _min_buy_prem or _short_ltp_p == 0.0)
            else:
                min_prem_ok = (_short_ltp_p >= _min_sell_prem or _short_ltp_p == 0.0)
            _min_floor = _min_buy_prem if 'BUY' in action else _min_sell_prem
            gates['Min Premium'] = (
                f'PASS (LTP≈{_short_ltp_p:.0f})' if min_prem_ok
                else f'FAIL (LTP={_short_ltp_p:.0f} < ₹{_min_floor:.0f} min for {action})'
            )
            ...
            try:
                ...
                # FIX-3: _is_tf must only be True when the operator's chosen action
                # matches the fade action suggested by _assess_trap_fade. The original
                # code set _is_tf=True whenever any trap existed, regardless of the
                # action typed in B7. This tagged non-fade positions as is_trap_fade=True,
                # suppressing STATE_EXIT_TRAP in _evaluate_lifecycle for those positions.
                _is_tf = (
                    _trap_type != 'NONE'
                    and _trap_fade.get('suggest', False)
                    and action == _trap_fade.get('fade_action', '').upper()
                )
            ...
            if txn == 'SELL' and hedge_token:
                # Hedge is always placed as MARKET order regardless of short leg order type.
                # Using short_ltp as a LIMIT price for the hedge is incorrect — the hedge
                # is a different strike with a different LTP, and forcing the short's price
                # on the hedge will cause the hedge order to sit unfilled or reject.
                hedge_params = {**short_params,
                                'tradingsymbol':   hedge_symbol,
                                'symboltoken':     hedge_token,
                                'transactiontype': hedge_txn,
                                'ordertype':       'MARKET',   # always MARKET for hedge leg
                                'price':           '0'}
                try:
            ...
            # FIX-1: Zero fill means broker never confirmed the order.
            # Proceeding with short_fill=0 sets net_premium=0 and max_risk=0,
            # making stoploss/target thresholds 0 × percentage = 0, so the
            # lifecycle fires incorrectly on the first non-zero proxy movement.
            # Hard-stop: alert, write FAILED status, and return without creating
            # active_position. The operator must verify the order in Angel One.
            if short_fill == 0.0:
                log.error(
                    '[OrderExecutor] Short fill is 0 after all retries. '
                    'Position NOT recorded. Verify order status in Angel One app '
                    'and close manually if the order was actually filled.'
                )
                self.sheets.write_execution_status(
                    status='FAILED',
                    error=(
                        'Short fill=0 after all retries. '
                        'Position not recorded — check Angel One order book immediately.'
                    )
                )
                self._alert(
                    '⚠️ ORDER FILL UNKNOWN\n'
                    f'Order ID: {short_order_id}\n'
                    'Short fill=0 after 5 attempts. '
                    'Check Angel One app immediately. Position NOT recorded.'
                )
                return True  # exit without creating active_position
    ...
    def _run_lifecycle_check(self, snapshot: dict):
        try:
            if not self.active_position: return
            pos = self.active_position
            state = self._evaluate_lifecycle(pos, snapshot)
            ...
            if state != old:
                ...
                self._alert(f'POSITION UPDATE\n{pos.get("action")} {pos.get("short_strike")}\n'
                           f'State: {old} → {state}')
                # PATCH-1: Split states into auto-exit vs advisory.
                # Auto-exit fires _exit_all_positions() immediately — no operator
                # action needed and no waiting for the next 5-minute cycle.
                # Advisory writes to Sheets + Telegram; operator uses EXIT ALL button.
                auto_exit_states = {
                    self.STATE_STOPLOSS,     # loss threshold breached
                    self.STATE_TARGET,       # premium target reached — lock it in
                    self.STATE_TIME_EXIT,    # 15:10 IST — exit before broker squareoff
                    self.STATE_HEDGE_FAILED, # unhedged short — immediate exit required
                    # FIX-2: STATE_EXIT_THETA → auto-exit.
                    # SELL in Expansion Favorable = premiums expanding fastest against
                    # the seller. A 5-min advisory delay can mean multiple stoploss
                    # multiples of loss before the operator acts.
                    self.STATE_EXIT_THETA,
                }
                advisory_states = {
                    self.STATE_EXIT_STRUCTURE, self.STATE_EXIT_VWAP,
                    self.STATE_EXIT_TRAP,
                    self.STATE_EXIT_OI,        self.STATE_EXIT_SR_RESET,
                    # FIX-2: PARTIAL states were orphaned — they fired from lifecycle
                    # but were in neither auto_exit nor advisory, so no Sheets status
                    # was written and no action prompt was shown to the operator.
                    self.STATE_PARTIAL_PROFIT, # 20% of premium reached — consider locking
                    self.STATE_PARTIAL_LOSS,   # 50% of max_risk reached — consider reducing
                }
                if state in auto_exit_states:
                    _reason_map = {
                        self.STATE_STOPLOSS:     'Stoploss threshold breached.',
                        self.STATE_TARGET:       'Target achieved — locking premium.',
                        self.STATE_TIME_EXIT:    '15:10 IST — exiting before broker squareoff.',
                        self.STATE_HEDGE_FAILED: 'Hedge failed — short leg unhedged.',
                    }
                    _reason = _reason_map.get(state, f'Auto-exit: {state}')
                    log.error(f'[OrderExecutor] AUTO-EXIT: {_reason}')
                    self._alert(
                        f'\u26a0\ufe0f AUTO-EXIT\n'
                        f'{pos.get("action")} {pos.get("short_strike")}\n'
                        f'State: {old} \u2192 {state}\n'
                        f'{_reason}'
                    )
                    self._exit_all_positions()
                    return
            # FIX-4: Advisory Sheets status refreshes every cycle in advisory state,
            # not only on state change. Without this, the EXECUTION tab showed the
            # last non-advisory status after the first cycle, giving the operator
            # no visible in-Sheets indication that the position needs attention.
            if state in advisory_states:
                self.sheets.write_execution_status(
                    status=state,
                    error=(
                        f'Advisory: Consider EXIT_ALL via button. '
                        f'Reason: {state}'
                    )
                )
        except Exception as e:
    ...
    def _exit_all_positions(self) -> bool:
        try:
            ...
            if pos.get('hedge_token'):
                ...
                # FIX-3: hedge_exit_txn must be sq_txn (the reversed direction).
                # pos['txn'] is the ENTRY direction. Using it here places a
                # second SELL on the hedge instead of closing (buying) it back.
                     'symboltoken': pos['hedge_token'], 'transactiontype': sq_txn}),
...
class ClaudeAnalyst:
    ...
    _SAFE_FALLBACK = {
        ...
        'mtf_alignment': 'GPT analysis not available — MTF context unavailable.',
        'premium_environment': {
            'market_condition': 'Neutral',
            'volatility_context': 'Unknown',
            'price_behavior': 'Range-bound',
    ...
    def __init__(self):
        ...
        _sp_hash = _hashlib.md5(CLAUDE_SYSTEM_PROMPT.encode()).hexdigest()[:8]
        log.info(f'[ChatGPT] System prompt: {len(CLAUDE_SYSTEM_PROMPT)} chars, '
                 f'hash={_sp_hash}, version={CONFIG.get("PROMPT_VERSION","?")}')
        if not self.enabled:
    ...
    def maybe_run(
        ...
        try:
            ...
            err_msg = str(e)
            log.warning(f'[ChatGPT] maybe_run() failed: {err_msg}\n{__import__("traceback").format_exc()}')
            # Disable GPT for the session on quota exhaustion — subsequent cycles
            # skip instantly instead of wasting 2 min on doomed retries.
            if 'insufficient_quota' in err_msg:
                self.enabled = False
                log.warning('[ChatGPT] QUOTA EXHAUSTED — GPT disabled for this session. '
                            'Add OpenAI credits at platform.openai.com to re-enable.')
            if self.last_analysis and self.last_run_ts != '—':
                try:
                    from datetime import datetime as _dt
                    _age = (now_ist().replace(tzinfo=None) -
                            _dt.strptime(self.last_run_ts, '%Y-%m-%d %H:%M:%S')).seconds // 60
                    if _age > int(CONFIG.get('interval_min', 5)):
                        log.warning(f'[ChatGPT] Returning cached analysis from '
                                    f'{self.last_run_ts} ({_age} min ago).')
                except Exception:
                    pass
            return self.last_analysis
    ...
    def _maybe_run(self, cycle_num, snapshot, metrics, signals, sr_event,
        ...
        # Morning brief — fires on the first cycle (cycle_num <= 1), not a time window.
        # The previous time-window (ist.minute < 20) almost never triggered because
        # startup (Sheets, instrument master, login, phase-1 checks) typically takes
        # 3-5+ minutes, pushing the first cycle past 09:20 IST every normal run.
        if not self.morning_done and cycle_num <= 1:
    ...
    def _call_morning_brief(self, snapshot: dict) -> dict:
        ...
        raw = self._call_claude(user_msg, max_tokens=600, expect_json=True)
    ...
    def _build_mtf_context(
        ...
        """
        result = {
            # mtf_5m intentionally omitted — never populated, not in system prompt
            'mtf_15m':       {},
            'mtf_30m':       {},
            'mtf_oi_trend':  {},
            'mtf_sr_context': {},
    ...
    def _call_eod(self, snapshot: dict):
        ...
        user_msg = (
            ...
            f"Previous day: High={self.prev_day.get('prev_high','N/A')} "
            f"Low={self.prev_day.get('prev_low','N/A')} "
            f"Close={self.prev_day.get('prev_close','N/A')} "
            f"Open={self.prev_day.get('prev_open','N/A')} "
            f"Range={self.prev_day.get('prev_range','N/A')} pts\n"
        ...
        for attempt in range(3):
            try:
                ...
                if r.status_code == 429:
                    # Distinguish billing quota exhaustion from temporary rate limiting.
                    # insufficient_quota will never succeed on retry — fail fast.
                    try:
                        err_body = r.json()
                    ...
                    if err_code == 'insufficient_quota':
                        raise RuntimeError(
                            f'HTTP 429 insufficient_quota: OpenAI billing quota exhausted — '
                            f'add credits at platform.openai.com. GPT disabled for this session.')
                ...
                if r.status_code >= 400:
                    ...
                    if r.status_code == 404:
                        raise RuntimeError(
                            f'HTTP 404: Model not found — check OPENAI_MODEL in SETTINGS '
                            f'(current: {payload.get("model","?")}). Full error: {err_text}')
                ...
                if finish_reason == 'length':
                    log.warning(f'[ChatGPT] Response truncated (finish_reason=length, '
                                f'max_tokens={max_tokens}) — analysis may be incomplete.')
    ...
    def _parse_json(raw: str) -> dict:
        try:
            ...
            return {
                'market_bias': 'Neutral',
                'current_situation': 'Analysis unavailable this cycle — JSON parse error. See logs.',
                'key_levels': {'support': '—', 'resistance': '—'},
                ...
                'score_agreement': False,
                'unusual_observation': None,
                'mtf_alignment': 'MTF context unavailable — JSON parse error.',
                'premium_environment': {
                    'market_condition': 'Neutral',
                    'volatility_context': 'Unknown',
                    'price_behavior': 'Range-bound',
    ...
    def _sanitize_output(analysis: dict) -> dict:
        ...
        def scrub(value: str) -> str:
            ...
            for phrase in FORBIDDEN:
                ...
                # (e.g. 'BUY' must not match 'buying'; 'SELL' must not match 'selling')
                # Multi-word phrases: exact substring match is already specific enough
                if ' ' in phrase:
...
class NiftyDashboardApp:
    ...
    def __init__(self):
        ...
        # ── FIX-5: Reconcile broker positions before first cycle ──────────────
    ...
    def _wait_for_market_open(self):
        ...
        while True:
            ...
            if ist.time() < dtime(9, 15) and ist.weekday() < 5:
                # Send pre-market Telegram alert ONCE while waiting — uses prev_day
                # data already loaded at startup. No GPT call needed here; GPT
                # morning brief fires at the first cycle after 09:15.
                if not _pre_market_sent and ist.time() >= dtime(9, 0):
                    try:
                        pd = self.prev_day
                        if pd:
                            ph  = pd.get('prev_high',  '—')
                            pl  = pd.get('prev_low',   '—')
                            pc  = pd.get('prev_close', '—')
                            po  = pd.get('prev_open',  '—')
                            prng = pd.get('prev_range', '—')
                            msg = (
                                f"📊 NIFTY PRE-MARKET BRIEF\n"
                                f"🕘 {ist.strftime('%d-%b-%Y %H:%M IST')}\n"
                                f"─────────────────────\n"
                                f"📅 Prev Day OHLC\n"
                                f"  Open:  {po}\n"
                                f"  High:  {ph}\n"
                                ...
                                f"  Resistance: {ph} (prev high)\n"
                                f"  Support:    {pl} (prev low)\n"
                                f"  Pivot:      {round((float(ph)+float(pl)+float(pc))/3, 2) if all(isinstance(x, (int,float)) for x in [ph,pl,pc]) else '—'}\n"
                                f"─────────────────────\n"
                                f"⏳ Market opens at 09:15 IST\n"
                                f"📈 GPT analysis will follow at first cycle."
                            )
                            self.telegram.send(msg)
                            log.info('[App] Pre-market Telegram brief sent.')
                        else:
                            log.warning('[App] Pre-market brief skipped — prev_day not available.')
                    except Exception as _e:
                        log.warning(f'[App] Pre-market brief failed: {_e}')
    ...
    def _check_existing_positions(self):
        """
        FIX-5: Startup position reconciliation.
        Queries the broker for live NIFTY option positions this script has
        no memory of (orphans from a prior crashed or restarted run).
        Logs a critical warning. Does NOT auto-exit — operator decides.
        """
        try:
            pos_data  = self.api_client.smart_connect.getPosition()
            positions = (pos_data.get('data') or []) if pos_data else []
            live = [
                p for p in positions
                if abs(int(p.get('netqty', 0) or 0)) > 0
                and 'NIFTY' in str(p.get('tradingsymbol', '')).upper()
            ]
            if live:
                log.error(
                    f'[Startup] {len(live)} live NIFTY position(s) detected '
                    f'that this script has no memory of — likely orphans.'
                )
                for p in live:
                    log.error(
                        f'  → {p.get("tradingsymbol","?")} '
                        f'netqty={p.get("netqty","?")} ltp={p.get("ltp","?")}'
                    )
                try:
                    self.sheets.write_execution_status(
                        status='WARNING',
                        error=(
                            f'{len(live)} orphan position(s) at startup. '
                            'Check Angel One app and close manually. Symbols: '
                            + ', '.join(p.get('tradingsymbol','?') for p in live)
                        )
                    )
                except Exception:
                    pass
            else:
                log.info('[Startup] No orphan NIFTY positions — clean start.')
        except Exception as e:
            ...
            if getattr(self.claude, 'enabled', False):
                try:
                    ...
                    fallback_prompt = (
                        ...
                        'Use the JSON schema from your system instructions exactly. '
                        'Fill current_situation with 1-2 sentences summarising why this cycle failed and current market context.'
                    )
        ...
        try:
            if snapshot and snapshot.get('bias') and snapshot.get('score') and snapshot.get('spot'):
                sig_eval = self.setup_evaluator.evaluate(
                    ...
                    days_to_expiry=days_to_expiry,
                    is_weekly=is_weekly,
                    lot_size=int(self.cfg.get('lot_size', 65)),
                )
